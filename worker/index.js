      ...
      async (payload) => {
        ...
        try {
          ...
          if (!aiService.aiAgentIds.has(message.user_id)) {
            const messageHistory = recentMessages; // Assuming recentMessages is the message history
            const lastUserMessage = message; // Assuming the current message is the last user message
            const agentPrompt = ''; // Placeholder for agent prompt, adjust as needed
...
async function init() {
  try {
    ...
    await Promise.all([
      supabaseService.fetchRecentMessages(),
      aiService.init(),
    ]);
