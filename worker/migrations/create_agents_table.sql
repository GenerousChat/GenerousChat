...
-- Create policy to allow all users to read agents
CREATE POLICY "Allow all users to read agents" ON public.agents
    FOR SELECT USING (true);

-- Create policy to allow only authenticated users to create agents
CREATE POLICY "Allow authenticated users to create agents" ON public.agents
    FOR INSERT TO authenticated USING (true);

-- Create policy to allow only authenticated users to update their own agents
CREATE POLICY "Allow authenticated users to update their own agents" ON public.agents
...
-- Create a function to get all agents
CREATE OR REPLACE FUNCTION public.get_agents()
RETURNS SETOF public.agents
LANGUAGE sql
SECURITY DEFINER
AS $$
    SELECT * FROM public.agents ORDER BY name;
$$;
