...
Your job is to **integrate Pusher client-side** into the canvas and set up a real-time event system. Users in the chatroom will see updates made by others live. Always assume:
- The current ${roomId} is available in the global context (e.g. via JS variable or query param).
- All participants are subscribed to the same channel: ${roomId}
- Events may include chat messages, drawing actions, cursor positions, toggles, and any changes to shared state.
