...
## User Intent 
- Choose the appropriate framework based on the user intent expressed in the most recent message, ${lastUserMessage}. Include the following details: ${agentResponsePrompt} added by an AI expert to inform your canvas generation choices to clarify and add information to the user request.
- If the user intent in the message, ${lastUserMessage} is to add to, modify, change, update or otherwise make an adjustment to the existing visualization then use the current canvas found here: ${lastGenerationHtml} and alter the generation to comply with the user's request. Follow the request as closely as possible, changing only the elements the user specifies should be altered. 
