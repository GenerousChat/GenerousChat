...
logger.info(`Initializing Supabase client with URL: ${config.supabase.url ? 'URL is set' : 'URL is NOT set'} and ${keyType} key: ${config.supabase.serviceKey || config.supabase.anonKey ? 'Key is set' : 'Key is NOT set'}`);
 ...
 */
async function setupSupabaseListeners(onMessageInserted, onParticipantJoined, onParticipantLeft) {
 ...
 */
async function saveGeneration(roomId, html, summary, createdBy, type, metadata) {
  try {
